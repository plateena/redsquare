// path: app/src/models/BaseModel.ts
...
export const BaseModel = <T>(
    schema: Schema<T>,
    options: { parseFilters: IFilterParser; appConfig?: any }
) => {
     ...
     */
    schema.statics.delete = async function <T>(id: number | string): Promise<T> {
     ...
     */
    schema.statics.search = async function <T>(this: Model<T>, req: Request): Promise<ISearch<T>> {
...
const BaseQuery = async <T>(
    ...
    const pipeline: any[] = [
        ...
        },
        {
            $facet: {
                data: [
                    ...(isNaN(page) ? [] : [{ $skip: skip }, { $limit: perPage }]),
        ...
        },
        {
            $addFields: {
                ...
                pagination: {
                    ...
                    per_page: perPage,
                    current_page: page,
                    last_page: page ? { $ceil: { $divide: ['$total.value', perPage] } } : undefined,
                },
            },
        },
        {
